...
class PyFAT:
    ...
    def load(self, rdir):
        ...
        self.detector.prepare(ctx_id=self.ctx_id)
    ...
    def insert(self, feat, idx):
        if self.feats is None:
            self.feats = np.zeros( (self.N, self.feat_dim), dtype=np.float32)
        self.feats[idx] = feat

    def cluster(self):
        self.labels = DBSCAN(eps=1.2, min_samples=2).fit(self.feats).labels_


    def gen_res(self, file_name):
        self.outliers = []
        self.label2idx = {}
        ...
        with open(file_name, 'w') as f:
            for label, idxlist in self.label2idx.items():
                _str = "\t".join([str(x) for x in idxlist])
                f.write(_str)
                f.write("\n")
