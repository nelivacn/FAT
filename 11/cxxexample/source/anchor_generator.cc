...
void AnchorGenerator::Predict(const cv::Mat& cls, const cv::Mat& reg, const cv::Mat& pts, float score_thresh, std::vector<Face>& result) const {
    ...
    //        for (int a = 0; a < anchor_num_; ++a) 
    //        {
    //            if(cls_data[(anchor_num_ + a)*step + id] > score_thresh) 
    //            //if(cls_data[(anchor_num_ + a)*step + id] > score_thresh) 
    //            {
    //                //std::cout <<"clsData[(anchor_num + a)*step + id]: "<< clsData[(anchor_num + a)*step + id] << std::endl;
    //                CRect2f box(i * anchor_stride_ + preset_anchors_[a][0],
    //                            j * anchor_stride_ + preset_anchors_[a][1],
    //                            i * anchor_stride_ + preset_anchors_[a][2],
    //                            j * anchor_stride_ + preset_anchors_[a][3]);

    //                //printf("box::%f %f %f %f\n", box[0], box[1], box[2], box[3]);
    //                CRect2f delta(reg_data[(a*4+0)*step+id],
    //                              reg_data[(a*4+1)*step+id],
    //                              reg_data[(a*4+2)*step+id],
    //                              reg_data[(a*4+3)*step+id]);
    //                //Anchor res;
    //                Face face;
    //                //res.anchor = cv::Rect2f(box[0], box[1], box[2], box[3]);
    //                bbox_pred(box, delta, face.bbox);
    //                face.score = cls_data[(anchor_num_ + a)*step+id];
    //                //res.center = cv::Point(i,j);
    //                //printf("center %d %d\n", i, j);
    //                std::vector<cv::Point2f> pts_delta(pts_length);
    //                for (int p = 0; p < pts_length; ++p) {
    //                    pts_delta[p].x = pts_data[a*pts_length*2+p*2+id];
    //                    pts_delta[p].y = pts_data[a*pts_length*2+p*2+1+id];
    //                }                		
    //                landmark_pred(box, pts_delta, face.pts);
    //                std::cout<<"Score: "<<face.score<<std::endl;
    //                result.push_back(face);
...
void AnchorGenerator::landmark_pred(const CRect2f& anchor, const std::vector<cv::Point2f>& delta, std::vector<cv::Point2f>& pts) const{
    ...
    //std::cout<<"ANCHOR: "<<w<<","<<h<<","<<x_ctr<<","<<y_ctr<<std::endl;
    for (std::size_t i = 0; i < delta.size(); ++i) {
        //std::cout<<"Pts delta "<<i<<" "<<delta[i].x<<","<<delta[i].y<<std::endl;
